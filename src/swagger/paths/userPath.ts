	...
	responses: {
		200: {
			...
			content: {
				"application/json": {
					schema: z.object({
						message: z.string().openapi({ example: "User list found" }),
						data: z.array(z.intersection(userIDSchema, userExample)),
					}),
				},
			},
		},
				...
				"application/json": {
					schema: z.object({
						message: z
							.string()
							.openapi({
								example: "You do not have permission to view this user list",
							}),
					}),
				},
			},
		...
		404: {
			...
			content: {
				"application/json": {
					schema: z.object({
						message: z.string().openapi({ example: "Token not found" }),
					}),
				},
			},
		},
			...
			content: {
				"application/json": {
					schema: z.object({
						...
						data: z.intersection(userIDSchema, userExample),
					}),
				},
			},
		},
	...
	responses: {
		204: {
			...
			content: {
				"application/json": {
					schema: z.object({
						message: z.string().openapi({ example: "User removed" }),
					}),
				},
			},
		},
